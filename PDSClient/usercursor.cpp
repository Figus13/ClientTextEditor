...
UserCursor::UserCursor(int siteId, QString nickname, int colorId): User(siteId, nickname, colorId)
{
...
UserCursor::UserCursor(User user): User(user.getSiteId(), user.getNickname(), user.getColorId()){
    label = new QLabel(nickname);
    label->hide();
    label->setTextFormat(Qt::RichText);
    label->setFixedHeight(20);
